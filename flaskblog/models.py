...
class Post(db.Model):
    ...
    # always use utc when saving dates and times to database so that they are always consistent 
    date_posted = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    content = db.Column(db.Text, nullable=False)
    # foreign key
    # 'user.id' - here 'u' small bcoz accessing a column from User model so user.id
